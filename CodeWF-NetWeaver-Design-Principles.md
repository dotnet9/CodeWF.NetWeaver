...
```csharp
// 使用常量代替魔数，便于统一管理和维护
[NetHead(NetConsts.LoginRequestObjectId, 1)]
...
所有对象 ID 均定义为常量，统一管理，避免魔数散落在代码各处：
...
**CodeWF.NetWrapper/SocketConstants.cs：**
...
public class SocketConstants
{
    ...
    ///     默认心跳超时（秒）
    /// </summary>
    public const int DefaultHeartbeatTimeout = 30;
    ...
    ///     默认包头长度
    /// </summary>
    public const int DefaultPacketHeaderLength = 23;
    ...
    ///     文件传输块大小（字节）
    /// </summary>
    public const int FileTransferBlockSize = 65536;

    /// <summary>
    ///     UDP 组播地址
    /// </summary>
    public const string DefaultMulticastAddress = "239.0.0.1";
}
```
...
│   ├── FileTransferStart.cs       # 文件传输开始
│   ├── FileTransferStartAck.cs    # 文件传输开始应答
│   ├── FileBlockData.cs           # 文件块数据
│   ├── FileBlockAck.cs            # 文件块传输应答
│   ├── FileTransferComplete.cs    # 文件传输完成
│   ├── FileTransferSession.cs     # 文件传输会话状态
│   └── FileTransferProgressEventArgs.cs # 文件传输进度事件
...
public class FileTransferProgressEventArgs
{
    ...
    /// 进度百分比（0.0 - 100.0）
    /// </summary>
    public double Progress => FileSize > 0 ? (TransferredBytes * 100.0 / FileSize) : 0;
}
```
...
1. **检查本地文件**：传输前检查本地是否已有部分文件
2. **计算已传输字节数**：通过文件大小或校验和确定已传输字节
3. **通知对端位置**：文件传输开始时，通过 `AlreadyTransferredBytes` 告知对端已传输位置
4. **从断点继续**：发送方从断点位置开始发送剩余数据块
5. **SHA256 校验**：传输完成后校验文件完整性

```csharp
/// <summary>
/// 获取已传输字节数（断点续传）
/// </summary>
private long GetExistingTransferBytes(string fileName, bool isUpload)
{
...
// 登录响应
[NetHead(NetConsts.LoginResponseObjectId, 1)]
...
{
    Console.WriteLine($"传输进度: {args.Progress:F2}% ({args.TransferredBytes}/{args.FileSize}字节)");
};
...
EventBus.Default.Subscribe<SocketCommand>(async (sender, cmd) =>
{
    ...
    {
        var transferStart = cmd.GetCommand<FileTransferStart>();
        Logger.Info($"收到文件传输请求: {transferStart.FileName}, 断点: {transferStart.AlreadyTransferredBytes}");
    }
});
```
...
5. **丰富的内置消息**：心跳检测、通用响应、文件传输等常用功能开箱即用
6. **断点续传文件传输**：支持大文件传输，网络中断后可继续传输，支持 SHA256 完整性校验
