...
- [文档] 修正依赖审计表中的 Avalonia 和 ProDataGrid 版本，改为与 `Directory.Packages.props` 一致。

## 4.0.0.7 (2026-09-22)

- [测试] 核心序列化测试和 Wrapper 文件链路测试扩展到 `net8.0;net10.0;net11.0`，覆盖库声明的全部目标框架。

## 4.0.0.6 (2026-09-22)

- [优化] TCP 服务端按客户端标识分片请求队列并使用固定 worker 处理，保持单客户端请求顺序，避免一个慢客户端阻塞所有客户端。

## 4.0.0.5 (2026-09-22)

- [文档] 同步多目标框架、序列化限制、Native AOT、文件系统沙箱和网络安全边界说明，并加入 CI 构建、测试和 AOT 发布检查。

## 4.0.0.4 (2026-09-22)

- [优化] 为 TCP/UDP 接收与命令处理增加有界队列，避免慢消费者导致无界内存增长。
- [修复] TCP/UDP 后台任务纳入停止流程并等待退出；单条命令处理器异常不会终止整个消费循环。

## 4.0.0.3 (2026-09-22)

- [修复] 文件上传和下载增加范围、分块长度、偏移和序号校验。
- [修复] 使用 `.part` 临时文件和哈希校验后的替换，修复空文件传输并避免损坏文件覆盖正式目标。

## 3.0.8 (2026-07-27)

- 💥[重构]-升级 `CodeWF.Log.Core` 至 `12.1.0.19` 并重新编译网络组件，适配静态 Logger 显式通知参数和精简后的方法签名；网络库日志默认不申请用户通知。
